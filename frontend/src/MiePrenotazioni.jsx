...
function MiePrenotazioni() {
    ...
    useEffect(() => {
        ...
        fetch(`${API}/prenotazioni/utente/${utente.id}`)
            ...
            .then(dati => { setPrenotazioni(dati); setCaricamento(false); });
    }, []);
                    ...
                    {prenotazioni.map((p) => (
                        <div key={p.id} style={styles.card}>
                            <div style={styles.cardHeader}>
                                ...
                                <span style={{ fontSize: '12px', fontWeight: 500, color: coloreStato(p.stato) }}>{p.stato}</span>
                            </div>
                            <p style={styles.dettagli}>{p.data} · {p.ora_inizio} - {p.ora_fine}</p>
                            <p style={styles.sport}>{p.sport}</p>
                            {p.stato !== 'cancellata' && (
                                <button onClick={() => handleCancella(p.id)} style={styles.bottoneCancella}>Cancella</button>
                            )}
                        </div>
                    ))}
                </div>
...
const styles = {
    ...
    vuoto: { color: '#888780', fontSize: '14px', textAlign: 'center', marginTop: '40px' },
    ...
    nomeCampo: { color: '#FAEEDA', fontSize: '15px', fontWeight: 500, margin: 0 },
    dettagli: { color: '#888780', fontSize: '13px', margin: '0 0 4px' },
    sport: { color: '#888780', fontSize: '12px', margin: '0 0 12px' },
    bottoneCancella: { background: 'none', border: '1px solid #F09595', borderRadius: '8px', padding: '6px 14px', color: '#F09595', fontSize: '12px', cursor: 'pointer' }
};
