...
function DettaglioCampo() {
    ...
    const handlePrenota = async () => {
        ...
        const risposta = await fetch(`${API}/prenotazioni`, {
            ...
            body: JSON.stringify({ id_utente: utente.id, id_campo: id, data, ora_inizio: oraInizio, ora_fine: oraFine })
        });

        const risultato = await risposta.json();
        if (!risposta.ok) { setMessaggio(risultato.errore); return; }
...
const styles = {
    ...
    nomeCampo: { color: '#FAEEDA', fontSize: '22px', fontWeight: 500, margin: '0 0 4px' },
    indirizzo: { color: '#888780', fontSize: '14px', margin: '0 0 4px' },
    sport: { color: '#888780', fontSize: '13px', margin: 0 },
    descrizione: { color: '#B4B2A9', fontSize: '14px', lineHeight: 1.5, marginBottom: '20px' },
    ...
    rigaDoppia: { display: 'flex', gap: '10px' },
    colonna: { flex: 1 },
    prezzoBox: { background: '#1C1F26', borderRadius: '20px', padding: '16px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' },
    etichettaPrezzo: { color: '#5F5E5A', fontSize: '12px', margin: '0 0 2px' },
