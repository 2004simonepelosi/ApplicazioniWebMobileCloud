...
async function avvia() {
    ...
    app.post('/utenti/registra', async (req, res) => {
        ...
        try {
            ...
            const stmt = db.prepare(`INSERT INTO utenti (nome, cognome, email, password, telefono) VALUES (?, ?, ?, ?, ?)`);
    ...
    app.post('/campi', (req, res) => {
        ...
        const stmt = db.prepare(`INSERT INTO campi (id_gestore, nome, sport, descrizione, indirizzo, prezzo_ora, max_giocatori, foto_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
    ...
    app.post('/prenotazioni', (req, res) => {
        ...
        if (data < oggi) {
            return res.status(400).json({ errore: 'Non puoi prenotare per una data passata' });
        }

        if (ora_fine <= ora_inizio) {
            return res.status(400).json({ errore: 'L\'ora di fine deve essere dopo l\'ora di inizio' });
        }
    ...
    `).get(id_campo, data, ora_fine, ora_inizio);
        ...
        const stmt = db.prepare(`INSERT INTO prenotazioni (id_utente, id_campo, data, ora_inizio, ora_fine, num_partecipanti, note) VALUES (?, ?, ?, ?, ?, ?, ?)`);
    ...
    app.post('/recensioni', (req, res) => {
        ...
        const stmt = db.prepare(`INSERT INTO recensioni (id_utente, id_campo, stelle, commento, data) VALUES (?, ?, ?, ?, ?)`);
